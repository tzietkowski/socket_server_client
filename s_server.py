...
class Server:
    ...
    def __init__(self, host = '127.0.0.1', port = 65432) -> None:
        ...
        self.commands = {'info': {'func': self.info ,'help': 'Info about the server version'},\
                        ...
                        'new_user': {'func': self.new_user ,'help':'Add new user: name_user:password'},\
    ...
    def login_user(self)-> bool:
        ...
        while not self.login:
            ...
            if message['command']['user'] in self.db.users.keys() and self.db.users[\
                ...
                self.user_admin = self.db.users[message['command']['user']].check_admin()
                return True
            else:
                self.server.send(json.dumps({\
                    'user': message['user'], 'answer': 'wrong login or password'}).encode())
        return False
